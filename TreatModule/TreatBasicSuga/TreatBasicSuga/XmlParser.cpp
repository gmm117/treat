...
{
	static CXmlParser theInstance;
	return theInstance;
}

void CXmlParser::AddVecXmlName()
{
	if (!m_vecXmlName.empty())
		m_vecXmlName.clear();
...
bool CXmlParser::SelectFileNode(int nIdx)
{
...
bool CXmlParser::SelectTypeNode(int nIdx, CXMLNode XmlNodeType, CString strStandDate)
{
...
void CXmlParser::LoadMapXMLInfo(long lIdx, CString strStandDate, std::vector<_S_BASIC_SUGA>& vecXMLParser)
{
	...
	{
		vecXMLParser.push_back(*iterVecBasicSuga);
	}
}

bool CXmlParser::InitEnvInfo(std::vector<long>& vecHospDiv, std::vector<CString>& vecStandDate)
{
...
bool CXmlParser::SetSequenceData(long lIdx, CString strStandDate, std::vector<_S_BASIC_SUGA> vecFixXMLParser)
{
	std::map<long, std::map<CString, std::vector<_S_BASIC_SUGA> > >::iterator iterMapTotXMLFile = m_mapTotBasicSuga.find(lIdx);
...
bool CXmlParser::SetXMLNode(long lIdx, CString strStandDate, std::vector<_S_BASIC_SUGA>& vecFixXMLParser)
{
	CString strFolderPath	= _T("");
	CString strXmlPath		= _T("");
	std::vector< CString >::iterator IterVecName = m_vecXmlName.begin();
...
bool CXmlParser::SetFileNode(long lIdx, CString strStandDate, std::vector<_S_BASIC_SUGA>& vecFixXMLParser)
{
...
bool CXmlParser::SetTypeNode(CXMLNode XmlNodeType, CString strStandDate, std::vector<_S_BASIC_SUGA>& vecFixXMLParser)
{
	CXMLIteratorNodes* iterXmlTypeNode = static_cast<CXMLIteratorNodes*>(XmlNodeType.GetChildren());
